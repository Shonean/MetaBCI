...
from metabci.brainda.algorithms.decomposition import SKLDA, STDA
...
print(
    "LDA分类正确率为：",
    np.sum(np.where(clf.predict(Xtest) - np.array([1, 1, 2, 2]), 0, 1)) / 4 * 100,
    "%",
)  # LDA预测值
...
print(
    "SKLDA分类正确率为：",
    np.sum(np.where(np.sign(clf2.transform(Xtest)) - np.array([-1, -1, 1, 1]), 0, 1))
    / 4
    * 100,
    "%",
)
...
y2 = np.hstack(
    (np.ones(100, dtype=int), np.ones(100, dtype=int) * -1)
)  # lables of the training set
...
print(
    "STDA分类正确率为：",
    np.sum(np.where(np.sign(clf3.transform(Xtest2)) - np.array([1, 1, 1, 1]), 0, 1))
    / 4
    * 100,
    "%",
)
