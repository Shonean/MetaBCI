...
def raw_hook(raw, caches):
    ...
    raw.filter(7, 55, l_trans_bandwidth=2, h_trans_bandwidth=5, phase="zero-double")
...
def train_model(X, y, srate=1000):
    ...
    model = FBTDCA(
        filterbank, padding_len=3, n_components=4, filterweights=np.array(filterweights)
    )
...
def time_feature(X, meta, dataset, event, channel, latency=0):
    ...
    Feature_R = TimeAnalysis(
        X, meta, dataset, event=event, latency=latency, channel=channel
    )

    plt.figure(1)
    # 计算模板信号调用TimeAnalysis.stacking_average()
    data_mean = Feature_R.stacking_average(
        np.squeeze(Feature_R.data[:, Feature_R.chan_ID, :]), _axis=0
    )
    ax = plt.subplot(2, 1, 1)
    sample_num = int(Feature_R.fs * Feature_R.data_length)
    # 画出模板信号及其振幅调用TimeAnalysis.plot_single_trial()
    loc, amp, ax = Feature_R.plot_single_trial(
        data_mean,
    ...
    Feature_R.plot_topomap(
        data_map, loc, fig=fig2, channels=Feature_R.All_channel, axes=ax
    )
    plt.show()
...
def frequency_feature(X, chan_names, event, SNRchannels, plot_ch, srate=1000):
    ...
    plt.plot(f, den)
    plt.text(12, den[f == 12][0], "{:.2f}".format(den[f == 12][0]), fontsize=15)
    plt.text(24, den[f == 24][0], "{:.2f}".format(den[f == 24][0]), fontsize=15)
    plt.text(36, den[f == 36][0], "{:.2f}".format(den[f == 36][0]), fontsize=15)
    plt.title("OZ FFT")
    plt.xlim([0, 60])
    plt.ylim([0, 4])
    plt.xlabel("fre [Hz]")
...
def time_frequency_feature(X, y, chan_names, srate=1000):
    ...
    plt.title("".join(("Scaleogram (ω = ", str(omega), " , ", "σ = ", str(sigma), ")")))
    plt.text(
        t_lim[1] + 0.04,
        f_lim[1] / 2,
        r"Power (\muV^2/Hz)",
    ...
    plt.plot(time, imagEnv, "b", label="image part")
    plt.plot(time, angle, "c", linestyle="-", label="angle part")
...
if __name__ == "__main__":
    ...
    # declare the dataset
    dataset = MetaBCIData(
        subjects=subjects, srate=srate, paradigm="ssvep", pattern="ssvep"
    )
    paradigm = SSVEP(
    ...
    X, y, meta = paradigm.get_data(
        dataset, subjects=subjects, return_concat=True, n_jobs=4, verbose=False
    )
