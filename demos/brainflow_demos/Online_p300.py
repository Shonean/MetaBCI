...
def P300read_data(run_files, chs, interval, row_plus_col, n_rounds=6):
    ...
    for run_file in run_files:  # different cnt
        ...
        label_loc = label_loc - np.expand_dims(label_loc[:, 0], 1).repeat(
...
def train_model(
    epochs_all, label_id_all, label_loc_all, signal_time=0.5, fs=1000, Analysis_down=1
):
    """train P300 model.
    -author: Ruixin Luo
    -Created on: 2022-12-06
    ...
    """
    # Filtering, all epoch
    epochs_all = filter_data(
        epochs_all, sfreq=fs, l_freq=1, h_freq=20, n_jobs=4, method="fir"
    )
    ...
    for i in range(n_commands):
        ...
        target_ind = np.where((label_small == row_ind) | (label_small == column_ind))[
            0
        ]  # shape(2 * n_rounds,)
        notarget_ind = np.where((label_small != row_ind) & (label_small != column_ind))[
            0
        ]  # shape(10 * n_rounds,)
        # Find target data
        ...
        for i_notar in notarget_ind:
            ...
            a_notarget = epochs[:, int(latency2) : int(latency2 + signal_time * fs)]
...
def LDA_predict(
    ...
    # prepare test data
    # filter
    epoch_test = filter_data(
        epoch_test, sfreq=fs, l_freq=1, h_freq=20, n_jobs=4, method="fir"
    )
    ...
    Xtest = np.zeros(
        [epoch_test.shape[0], int(signal_time * fs), row_plus_col, stim_round]
    )
    ...
    for m in range((row_plus_col)):
        for j in range((stim_round)):
            data = Xtest_r[:, m, j]
            rr[m, j] = (
                np.dot(data, np.transpose(LDA_model.coef_)) + LDA_model.intercept_
            )
...
class FeedbackWorker(ProcessWorker):
    ...
    def pre(self):
        ...
        for i_cross in range((n_cross)):
            ...
            train_run_files = [
                "{:s}\\{:d}.cnt".format(self.filepath, run) for run in ind_train
            ]  # train
            test_run_files = [
                "{:s}\\{:d}.cnt".format(self.filepath, run) for run in ind_test
            ]  # test
        ...
        all_run_files = [
            "{:s}\\{:d}.cnt".format(self.filepath, run) for run in all_runs
        ]  # train
        epochs_all, label_id_all, label_loc_all, ind_pickChannel = P300read_data(
    ...
    def consume(self, data):
        ...
        label_loc_test = [
            i
            for i in range(1, label_all.shape[0])
            if label_all[i] > 0 and label_all[i - 1] == 0
        ]
        label_id_test = label_all[label_loc_test].astype(int)
        label_loc_test = np.array(label_loc_test)
        # predict
        p_labels = LDA_predict(
            LDA_model=self.model_p300,
...
if __name__ == "__main__":
    ...
    marker = Marker(
        interval=command_interval, srate=srate, events=stim_labels
    )  # 打标签全为1
    ...
    ns.register_worker(
        feedback_worker_name, worker, marker
