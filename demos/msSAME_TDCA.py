...
from metabci.brainda.algorithms.transfer_learning import MSSAME, SAME
from metabci.brainda.algorithms.utils.model_selection import (
    generate_shuffle_indices,
    match_shuffle_indices,
    set_random_seeds,
)
...
paradigm = SSVEP(
    ...
    srate=250,
    events=events,  #  events: Frequency ascending order
)
...
X, y, meta = paradigm.get_data(
    dataset, subjects=[23], return_concat=True, n_jobs=None, verbose=False
)
...
indices = generate_shuffle_indices(
    meta, n_splits=n_splits, train_size=1, validate_size=1, test_size=4
)  # Nt = 2
...
for k in range(n_splits):
    ...
    estimator = FBTDCA(
        filterbank, padding_len=5, n_components=8, filterweights=np.array(filterweights)
    )
    ...
    p_labels_withSAME = estimator.fit(X_train_new1, y_train_new1, Yf=Yf).predict(
        X_test
    )  # with SAME
    ...
    mssame = MSSAME(
        fs=250, Nh=5, flist=freq_list, plist=phase_list, n_Aug=4, n_Neig=12
    )  # When n_Neig=1, the result is similar to SAME
    ...
    p_labels_withmsSAME = estimator.fit(X_train_new2, y_train_new2, Yf=Yf).predict(
        X_test
    )  # with msSAME
