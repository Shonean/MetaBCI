...
# Date: 2024/9/1
"""
This is a demo for the CE algorithm.
"""
...
paradigm = SSVEP(srate=srate, channels=channels, events=events)
...
models = OrderedDict(
    [
        ("fbscca", FBSCCA(filterbank, filterweights=filterweights)),
        ("fbecca", FBECCA(filterbank, filterweights=filterweights)),
        ("fbdsp", FBDSP(filterbank, filterweights=filterweights)),
        ("fbtrca", FBTRCA(filterbank, filterweights=filterweights)),
        (
            "fbtdca",
            FBTDCA(
                filterbank,
...
for model_name in models:
    ...
    for duration in np.round(np.arange(0.2, 1.1, 0.1), 2):
        ...
        Yf = generate_cca_references(
            freqs, srate, duration, phases=None, n_harmonics=n_harmonics
        )
        ...
        if model_name == "fbtdca":
            filterX, filterY = (
                np.copy(
                    X[
                        ...,
                        int(srate * delay) : int(srate * (delay + duration))
                        + padding_len,
                    ]
                ),
                np.copy(y),
            ...
            filterX, filterY = (
                np.copy(X[..., int(srate * delay) : int(srate * (delay + duration))]),
                np.copy(y),
            )

        filterX = filterX - np.mean(filterX, axis=-1, keepdims=True)

        train_ind, validate_ind, _ = match_loo_indices(5, meta, loo_indices)
    ...
    for i in range(0, 40):
        ...
        filterX = filterX - np.mean(filterX, axis=-1, keepdims=True)
        _, _, test_ind = match_loo_indices(5, meta, loo_indices)
        ...
        if model_name == "fbtdca":
            trail = bufferX[:, :, 0 : int(srate * default_duration) + padding_len]
        else:
