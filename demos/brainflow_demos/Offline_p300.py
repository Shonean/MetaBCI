...
def Time_Analysis(
    ...
    # Filtering, all epoch
    epochs_all = filter_data(
        epochs_all, sfreq=fs, l_freq=1, h_freq=10, n_jobs=4, method="fir"
    )
    n_commands = label_id_all.shape[0]
    ...
    for i in range(n_commands):
        ...
        for i_tar in target_ind:
            latency1 = label_latency[i_tar]
            a_target = epochs[
                :, int(latency1 - baseline_time * fs) : int(latency1 + signal_time * fs)
            ]
            targets.append(np.expand_dims(a_target, 0))
        # Find non-target data
        for i_notar in notarget_ind:
            latency2 = label_latency[i_notar]
            a_notarget = epochs[
                :, int(latency2 - baseline_time * fs) : int(latency2 + signal_time * fs)
            ]
            notargets.append(np.expand_dims(a_notarget, 0))
    # shape(2 * n_rounds *n_commands, n_channels, n_times)  e.g, 864, 6, 750
    targets = np.concatenate(targets)
...
if __name__ == "__main__":
    ...
    for i_cross in range((n_cross)):
        ...
        train_run_files = [
            "{:s}\\{:d}.cnt".format(filepath, run) for run in ind_train
        ]  # train
        test_run_files = [
            "{:s}\\{:d}.cnt".format(filepath, run) for run in ind_test
        ]  # test
    ...
    draw_run_files = [
        "{:s}\\{:d}.cnt".format(filepath, run) for run in draw_runs
    ]  # train
