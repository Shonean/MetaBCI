    ...
    for i in range(n_harmonics):
        Yf.append(
            np.stack(
                [
                    np.sin(
                        2 * np.pi * (i + 1) * freqs * t + (i + 1) * np.pi * phases
                    ),  # different phases pre-defined
                    np.cos(2 * np.pi * (i + 1) * freqs * t + (i + 1) * np.pi * phases),
                ],
                axis=1,
            )
        )
...
Yf = generate_mscca_references(
    freq_list, srate=250, T=0.5, phases=phase_list, n_harmonics=5
)
...
X, y, meta = paradigm.get_data(
    dataset, subjects=[1], return_concat=True, n_jobs=None, verbose=False
)
