...
def trial_hook(raw, caches):
    ...
    raw.filter(1, 20, l_trans_bandwidth=1, h_trans_bandwidth=4, phase="zero-double")
...
X, y, meta = paradigm.get_data(dataset, subjects=subject, verbose=False)
...
for k in range(k_loo):
    train_id, val_id, test_id = match_char_kfold_indices(k, meta, indices)
    ...
    for test_i in range(len(X_test_sort)):
        ...
        decode_key = TimeDecodeTool1.decode(
            label_test[test_i], test_feature, fold_num=5, paradigm=dataset.paradigm
        )
...
print(
    "Average speller decode accuracy of subject {} is: {}".format(
        subject, np.mean(speller_accs)
    )
)
